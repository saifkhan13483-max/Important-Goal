...
import {
  ...
  User, Globe, Lock, ChevronRight, Image as ImageIcon, Menu,
} from "lucide-react";
...
type NavSection = "overview" | "plan" | "config" | "links";

const NAV_ITEMS: { id: NavSection; label: string; icon: React.ElementType; description: string }[] = [
  { id: "overview",  label: "Overview",       icon: BarChart3,    description: "Account profile & activity" },
  { id: "plan",      label: "Plan & Features", icon: Crown,        description: "Subscription & feature matrix" },
  { id: "config",    label: "Configuration",   icon: Settings,     description: "Environment variables" },
  { id: "links",     label: "Quick Links",     icon: ExternalLink, description: "External dashboards" },
];
...
function AdminSidebar({
  ...
  user: { name: string; email: string; avatarUrl?: string | null; plan?: PlanTier | null };
  configPct: number;
  onBack: () => void;
  mobileOpen: boolean;
  onMobileClose: () => void;
  ...
  return (
    <>
      ...
      <aside className={cn(
        "fixed top-0 left-0 h-full w-60 bg-background border-r border-border flex flex-col z-30 transition-transform duration-200",
        ...
        <nav className="flex-1 py-4 px-2 space-y-0.5 overflow-y-auto">
          ...
          {NAV_ITEMS.map(({ id, label, icon: Icon, description }) => (
            ...
            >
              <Icon className={cn("w-4 h-4 flex-shrink-0", active === id ? "text-white" : "text-muted-foreground group-hover:text-foreground")} />
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium leading-tight">{label}</p>
                <p className={cn("text-[10px] leading-tight truncate mt-0.5", active === id ? "text-violet-200" : "text-muted-foreground/60")}>{description}</p>
              </div>
              {id === "config" && configPct < 100 && (
                <span className={cn("w-2 h-2 rounded-full flex-shrink-0", active === id ? "bg-amber-300" : "bg-amber-400")} />
              )}
            </button>
          ))}
          ...
          <div className="pt-4">
            ...
            <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider px-3 mb-2">App</p>
            <button
              onClick={onBack}
              className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left text-muted-foreground hover:bg-muted/60 hover:text-foreground transition-colors"
              data-testid="sidebar-back-to-app"
            >
              <ArrowLeft className="w-4 h-4 flex-shrink-0" />
              <p className="text-sm font-medium">Back to App</p>
            </button>
          </div>
        </nav>
          ...
          <div className="flex items-center gap-2.5 rounded-lg bg-muted/40 px-3 py-2.5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="flex items-center gap-1 mt-0.5">
                <Badge className={cn("text-[8px] px-1 py-0 leading-tight", PLAN_COLORS[user.plan ?? "free"])}>
...
export default function AdminPage() {
  ...
  const goalsQuery   = useQuery({ queryKey: ["admin-goals",   user?.id], queryFn: () => getGoals(user!.id),   enabled: !!user?.id });
  const systemsQuery = useQuery({ queryKey: ["admin-systems", user?.id], queryFn: () => getSystems(user!.id), enabled: !!user?.id });
  const checkinsQuery= useQuery({ queryKey: ["admin-checkins",user?.id], queryFn: () => getCheckins(user!.id),enabled: !!user?.id });
  ...
  const currentStreak = useMemo(() => {
    ...
    let best = 0;
    for (const sys of systems.filter(s => s.active !== false)) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background p-6">
        <Card className="w-full max-w-md">
          ...
          <CardContent><Button className="w-full" onClick={() => navigate("/login")}>Go to Login</Button></CardContent>
        </Card>
      </div>
    );
  }
  ...
  async function handleSavePlan() {
    ...
    setSaving(true);
    try {
      const updated = await updateUser(user.id, { plan: selectedPlan as PlanTier, planUpdatedAt: new Date().toISOString() });
      setUser(updated);
      queryClient.invalidateQueries();
      setSelectedPlan("");
      toast({ title: `Plan updated to ${planDisplayName(selectedPlan as PlanTier)}` });
    } catch {
  ...
  const planUpdatedDate= user.planUpdatedAt ? new Date(user.planUpdatedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }) : null;
  const currentFeatures= getPlanFeatures(user.plan);
  const currentLimits  = getPlanLimits(user.plan);

  const featureMatrixRows: { label: string; key: keyof ReturnType<typeof getPlanFeatures> | "__goal" | "__system" }[] = [
    { label: "Goal limit",            key: "__goal" },
    { label: "System limit",          key: "__system" },
    { label: "AI Coach",              key: "aiCoach" },
    { label: "AI msgs/day",           key: "aiCoachDailyLimit" },
    ...
    switch (activeSection) {
      ...
      case "overview": return (
        <div className="space-y-6">
          <SectionHeader icon={BarChart3} title="Overview" description="Your account profile and activity at a glance" />
                ...
                <div className="space-y-1.5 text-sm">
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-1">
                      <span className="font-mono text-xs text-muted-foreground truncate max-w-[110px]" data-testid="text-user-id">{user.id}</span>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="rounded-lg border bg-muted/30 p-3 space-y-1.5">
                  ...
                  <p className="text-[10px] text-muted-foreground">{doneCheckins} done of {totalCheckins} total check-ins</p>
                </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {systemsByFrequency.map(([freq, count]) => (
                        <Badge key={freq} variant="outline" className="text-[10px] capitalize">{freq} · {count}</Badge>
                      ))}
                    </div>
                  </div>
                )}
          ...
          <Card>
            <CardContent className="p-4">
              ...
              {configPct < 100 && (
                <button className="text-[10px] text-violet-500 hover:underline mt-1.5 block" onClick={() => setActiveSection("config")}>
                  View Configuration → see what's missing
                </button>
              )}
            </CardContent>
          </Card>
      ...
      case "plan": return (
        <div className="space-y-6">
          <SectionHeader icon={Crown} title="Plan & Features" description="Manage subscription and see what each plan unlocks" />
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                  ...
                  {PLAN_TIERS.map(tier => (
                    <button
                      ...
                      className={cn(
                        ...
                        selectedPlan === tier ? "border-primary bg-primary/5 ring-1 ring-primary" : "border-border hover:bg-muted/30",
                    ...
                    >
                      <div className="flex items-center gap-1 mb-0.5">
                        ...
                        <span className="text-sm font-semibold">{planDisplayName(tier)}</span>
                        {tier === (user.plan ?? "free") && <Badge variant="secondary" className="text-[8px] px-1 py-0 ml-1">current</Badge>}
                      </div>
                ...
                <Button className="w-full" onClick={handleSavePlan} disabled={!selectedPlan || saving} data-testid="button-save-plan">
      ...
      case "config": return (
        <div className="space-y-4">
          <SectionHeader icon={Settings} title="Configuration" description="Environment variables set in Replit Secrets" />
          ...
          <div className="rounded-xl border bg-muted/30 p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">{configuredCount} of {allEnvValues.length} variables configured</p>
            </div>
          ...
          <ConfigGroup title="Firebase (Required)" icon={Database} items={[
          ...
          <ConfigGroup title="EmailJS — Transactional Email" icon={Mail} items={[
          ...
          <ConfigGroup title="Optional Services" icon={Globe} items={[
      ...
      case "links": return (
        <div className="space-y-6">
          <SectionHeader icon={ExternalLink} title="Quick Links" description="Open connected service dashboards in a new tab" />
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-sm font-medium flex items-center gap-2">
                <Database className="w-4 h-4 text-muted-foreground" /> Firebase Project Info
              </CardTitle>
            </CardHeader>
  ...
  return (
    <div className="min-h-screen bg-muted/30 dark:bg-background flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Mobile top bar */}
        <div className="h-14 flex items-center gap-3 px-4 border-b bg-background lg:hidden sticky top-0 z-10">
          <Button variant="ghost" size="icon" className="w-8 h-8" onClick={() => setSidebarOpen(true)} data-testid="button-mobile-menu">
          ...
          <Badge variant="outline" className="text-[10px] ml-auto capitalize">{NAV_ITEMS.find(n => n.id === activeSection)?.label}</Badge>
        </div>
